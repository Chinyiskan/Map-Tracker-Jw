        ...
        Object.keys(rules).forEach(fieldName => {
            ...
            fieldRules.forEach(rule => {
                if (typeof rule === 'string') {
                    // Regla simple
                    const ruleFunction = defaultRules[rule];
                    if (ruleFunction) {
                        let error = null;
                        if (rule === 'minLength') {
                            // Para minLength necesitamos un parámetro adicional, usar 1 como default
                            error = ruleFunction(fieldValue, fieldName, 1);
                        }
                        else if (rule === 'required') {
                            error = ruleFunction(fieldValue, fieldName);
                        }
                        else if (rule === 'email' || rule === 'date') {
                            error = ruleFunction(fieldValue);
                        }
                        else {
                    ...
                    const ruleFunction = defaultRules[type];
                    if (ruleFunction) {
                        const error = ruleFunction(fieldValue, fieldName, ...Object.values(params));
                        if (error) {
                            errors[fieldName] = error;
                        }
                    }
