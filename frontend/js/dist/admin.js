...
export const AdminManager = {
    ...
    async loadSalidas(filters = {}) {
        try {
            ...
            if (filters.capitan_id)
                params.append('capitan_id', filters.capitan_id);
            if (filters.barrio_asignado)
                params.append('barrio_asignado', filters.barrio_asignado);
            if (filters.dia_semana)
                params.append('dia_semana', filters.dia_semana);
    ...
    _calculateStats(reportes) {
        ...
        return {
            ...
            porBarrio,
            porMes,
            barrioMasActivo: Object.keys(porBarrio).reduce((a, b) => porBarrio[a] > porBarrio[b] ? a : b, ''),
    ...
    async handleSalidaSubmit(event) {
        ...
        try {
            ...
            UI.showNotification(isEdit ? 'Salida actualizada correctamente' : 'Salida creada correctamente', 'success');
    ...
    async deleteSalida(id) {
        try {
            const confirmed = await this._showConfirmDialog('Confirmar eliminación', '¿Estás seguro de que quieres eliminar esta salida?');
            if (!confirmed)
                return;
    ...
    */
    ...
    initTimeSelector() {
        ...
        if (!timeInput || !horaHiddenInput || !minutosHiddenInput)
            return;
