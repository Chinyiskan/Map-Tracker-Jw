...
import type { JSONParseOptions, JSONStringifyOptions, StorageOptions } from '../types/index.js';

export interface JSONUtilsInterface {
...
export const JSONUtils: JSONUtilsInterface = {
   ...
   */
  getFromStorage<T = any>(key: string, defaultValue: T | null = null): T | null {
    try {
      const stored = localStorage.getItem(key);
      if (!stored) return defaultValue;
      
      return (this.safeParse as any)(stored, defaultValue);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Error desconocido';
   ...
   * @returns Datos parseados o valor por defecto
   */
  getFromSession<T = any>(key: string, defaultValue: T | null = null): T | null {
    try {
...
declare global {
  interface Window {
    safeJSONParse: typeof JSONUtils.safeParse;
    safeJSONStringify: typeof JSONUtils.safeStringify;
  }
}
