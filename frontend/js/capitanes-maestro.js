...
async function guardarCapitanMaestro(e) {
  ...
  try {
    ...
    if (capitanEditando) {
      // Actualizar
      const id = capitanEditando.id || capitanEditando.ID;
      response = await fetch(`/api/capitanes/${id}`, {
        method: 'PUT',
        headers: {
      ...
      // Crear nuevo
      response = await fetch('/api/capitanes', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
    ...
    if (!response.ok || !result.success) {
      console.error('Error al guardar:', result.error || result.message);
      mostrarNotificacion(result.message || 'Error al guardar el capitán', 'error');
      return;
    }
    
    mostrarNotificacion(
      capitanEditando ? 'Capitán actualizado correctamente' : 'Capitán creado correctamente',
      'success'
    );
...
function editarCapitanMaestro(id) {
  ...
  if (header) header.textContent = 'Editar Capitán';
...
async function eliminarCapitanMaestro(id) {
  ...
  try {
    const response = await fetch(`/api/capitanes/${id}`, {
      method: 'DELETE'
    });
    ...
    if (!response.ok || !result.success) {
      console.error('Error al eliminar:', result.error || result.message);
      mostrarNotificacion(result.message || 'Error al eliminar el capitán', 'error');
      return;
    }
...
function limpiarFormularioMaestro() {
  ...
  const header = document.querySelector('#form-capitan-maestro').closest('.card').querySelector('h3');
  if (header) header.textContent = 'Gestionar Capitán';
}
...
export {
  cargarCapitanesMaestro,
  mostrarCapitanesMaestro,
  editarCapitanMaestro,
  eliminarCapitanMaestro
};